...
void writeFunction(FILE *outFile, function *f)
{
    ...
    // Adjust the base and top stack pointers
    // Put the caller base stack pointer at the top of the stack
    fprintf(outFile, "\tpushq %%rbp\n");
    // New base stack pointer is now the top of the stack (new stack for the called function)
    ...
    // Restore the old top of stack pointer
    fprintf(outFile, "\tmovq %%rbp, %%rsp\n");
    // The top of stack should be the original caller base pointer because every other value should already be popped off
    fprintf(outFile, "\tpop %%rbp\n");

    fprintf(outFile, "\tret\n"); // Functions require return statements at the end
...
void writeExpression(FILE *outFile, expression *e)
{
    if (e->type == INT)
    {
        ...
        fprintf(outFile, "\tmovq $%d, %%rax\n", e->value);
    }
    else if (e->type == VARIABLE)
    {
        fprintf(outFile, "\tmovq -%d(%%rbp), %%rax\n", e->var->ebp_offset);
    }
